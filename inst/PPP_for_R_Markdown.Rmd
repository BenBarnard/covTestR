...
<div class = "notes">
Schott in 2001 proposed a Wald test. This Wald test is only valid when the pooled sample covariance matrix is nonsingular or when n is much greater than m. This really won't work for high dimensional data, but for testing many populations it might be workable.
</div>
...
<div class = "notes">
Srivastava et al. proposed a test in 2014 that had the same form as Schott 2007 but used an unbiased consistent estimator for a hat 2 i. V i is still the numerator of the popuation sample covariance matrix.
</div>
...
$$\boldsymbol{A}_i = \left(\boldsymbol{X}_i - \overline{\boldsymbol{X}_i} \right) \left( \boldsymbol{X}_i - \overline{\boldsymbol{X}_i} \right)' \quad \quad \boldsymbol{D}_i = diag \left(  a_{ij}\right)$$
<div class = "notes">
If the numerator for the sample covariance for our data is the matrix x i - x bar transpose times x i - x bar then A i is xi - x bar times x i - x bar transpose. D i is the diagonalization of the elements of the matrix A i. 
</div>
...
<div class = "notes">
h tilde the noise reduced first eigenvector is solved using the noised reduced first eigenvalue. h tilde star is the max of the scalar from the first noise reduced eigenvectors times each other or the inverse.  
</div>
...
- Let $\boldsymbol{V}$ represent the scatter matrix of $\boldsymbol{X}$

- Let $\boldsymbol{V} = \boldsymbol{UDU}^T$ represent the singular value decompostion of $\boldsymbol{V}$.
...
- Generating the $k$ populations from $\mathcal{N}_p \left( \boldsymbol{0}, \boldsymbol{\Sigma}_i \right)$.

- Number of repetitions is 100,000.
...
- Generating the $k$ populations from $\mathcal{N}_p \left( \boldsymbol{0}, \boldsymbol{\Sigma}_i \right)$, and $\mathcal{N}_p \left( \boldsymbol{0}, \boldsymbol{\Sigma}_i * c \right)$.
